...
#' @rdname new-metric
#' @export
new_pronostic_survival_metric <- function(fn, direction) {
  new_metric(fn, direction, class = "pronostic_survival_metric")
}
...
format.metric <- function(x, ...) {
  ...
  metric_type <-
    switch(
      ...
      "pronostic_survival_metric" = "pronostic survival metric",
      "metric"
    )
